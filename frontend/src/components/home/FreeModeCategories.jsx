// src/components/home/FreeModeCategories.jsx
...
export const FreeModeCategories = () => {
            ...
            return (
              <Link to={link} key={`${category}-${topic}`} style={{ textDecoration: 'none' }}>
                <div style={{ background: darkMode ? '#16213e' : 'white', padding: 20, borderRadius: 16, boxShadow: '0 2px 8px rgba(0,0,0,0.1)', display: 'flex', flexDirection: 'column', height: '100%', wordBreak: 'break-word' }}>
                  ...
                  <h3 style={{ color: darkMode ? headingColor : '#1e3c72', fontSize: 'clamp(16px, 4vw, 18px)', marginBottom: 8 }}>{topic}</h3>
                  <p style={{ color: darkMode ? '#aaa' : '#666', fontSize: 13, marginBottom: 12 }}>{infoText}</p>
                  {premiumTag && premiumTag}
                  <div style={{ marginTop: 'auto' }}>
                    <button style={{ width: '100%', background: '#1e3c72', color: 'white', border: 'none', padding: '10px', borderRadius: 10, cursor: 'pointer', fontWeight: 'bold', fontSize: 14 }}>
                      View Exams
                    </button>
                  </div>
                </div>
  ...
  if (loading || categoriesLoading) {
    return <LoadingWithBar message="Loading free mode courses" />;
  }
  ...
  // 👇 FLOATING BACK BUTTON – GO TO HOME (UPDATED)
      ...
      <div style={{ maxWidth: 1200, margin: '0 auto' }}>
        ...
        <div style={{ textAlign: 'center', marginBottom: 20 }}>
          ...
          <p style={{ color: darkMode ? '#aaa' : '#666', fontSize: 'clamp(14px, 4vw, 16px)' }}>
            Take each examination ONCE. Upgrade to retake exams and unlock all questions!
          </p>
        </div>
          ...
          <>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', rowGap: 24, columnGap: 24 }}>
              {displayCategories.map((cat) => (
                <Link to={`/courses/${cat.slug}/free`} key={cat.slug} style={{ textDecoration: 'none' }}>
                  <div style={{ 
                  ...
                  onMouseLeave={(e) => e.currentTarget.style.transform = 'translateY(0)'}>
                    ...
                    <h2 style={{ color: darkMode ? headingColor : '#1e3c72', fontSize: 'clamp(18px, 4vw, 20px)', marginBottom: 8 }}>{cat.name}</h2>
                    ...
                    <button style={{ marginTop: 16, background: '#1e3c72', color: 'white', border: 'none', padding: '10px 20px', borderRadius: 30, cursor: 'pointer', fontWeight: 'bold', fontSize: 14, width: '100%' }}>
                      Explore Courses
                    </button>
                  </div>
                </Link>
