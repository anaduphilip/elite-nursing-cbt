...
export const WeeklyQuizTab = ({
  ...
  return (
    <div style={{ padding: '10px 0', maxWidth: '100%', boxSizing: 'border-box' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20, flexWrap: 'wrap', gap: 12 }}>
        <h3 style={{ color: headingColor, margin: 0 }}>Manage Weekly Quizzes</h3>
        <button
          onClick={() => setShowQuizForm(!showQuizForm)}
          style={{
            background: showQuizForm ? '#dc3545' : '#2E7D64',
            color: 'white',
            padding: '10px 24px',
            border: 'none',
            borderRadius: 8,
      ...
      {showQuizForm && (
        <div style={{
          ...
          <h4 style={{ color: headingColor, marginBottom: 20, fontSize: 18 }}>
            {editingQuizId ? '✏️ Edit Quiz' : '📝 New Weekly Quiz'}
          </h4>

          {/* ===== RESPONSIVE GRID FOR FORM FIELDS ===== */}
          <div className="wq-grid-2" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '18px 24px', marginBottom: 18 }}>
            <div>
              <label style={labelStyle}>Quiz Title <span style={{ color: '#dc3545' }}>*</span></label>
              <input
                type="text"
            ...
            <div>
              <label style={labelStyle}>Week Number <span style={{ color: '#dc3545' }}>*</span></label>
              <input
                type="number"
                placeholder="1, 2, 3..."
          ...
          <div style={{ marginBottom: 18 }}>
            <label style={labelStyle}>Description <span style={{ color: '#999', fontWeight: 'normal' }}>(optional)</span></label>
            <input
              type="text"
          ...
          <div style={{ marginBottom: 18 }}>
            <label style={labelStyle}>Instructions <span style={{ color: '#999', fontWeight: 'normal' }}>(shown before quiz starts)</span></label>
            <textarea
          ...
          <div className="wq-grid-2" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '18px 24px', marginBottom: 18 }}>
            <div>
              <label style={labelStyle}>Start Date <span style={{ color: '#999', fontWeight: 'normal' }}>(optional)</span></label>
              <input
                type="datetime-local"
                value={quizStartDate}
            ...
            <div>
              <label style={labelStyle}>End Date <span style={{ color: '#999', fontWeight: 'normal' }}>(optional)</span></label>
              <input
                type="datetime-local"
                value={quizEndDate}
          ...
          <div className="wq-grid-2" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '18px 24px', marginBottom: 20 }}>
            <div>
          ...
          <div style={{ marginBottom: 20, borderTop: `2px solid ${darkMode ? '#444' : '#e0e0e0'}`, paddingTop: 20 }}>
            ...
            <div style={{
              ...
              <div className="wq-grid-options" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(140px, 1fr))', gap: '10px', marginBottom: 14 }}>
                  ...
                  >
                    ...
                    <div style={{ display: 'flex', gap: 6, flexShrink: 0 }}>
                      <button
                        ...
                        style={{ background: '#ffc107', color: '#333', padding: '4px 12px', border: 'none', borderRadius: 4, cursor: 'pointer', fontSize: 12, fontWeight: 500 }}
                      >
                        Edit
                      </button>
                      <button
                        onClick={() => handleDeleteQuestion(idx)}
                        style={{ background: '#dc3545', color: 'white', padding: '4px 12px', border: 'none', borderRadius: 4, cursor: 'pointer', fontSize: 12, fontWeight: 500 }}
                      >
                        Delete
                      </button>
                    </div>
      ...
      {loadingQuizzes ? (
        ...
        <div className="wq-card-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: 20 }}>
            ...
            return (
              ...
              >
                <div style={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'space-between', alignItems: 'flex-start', gap: 8 }}>
                  <div style={{ flex: 1, minWidth: '150px' }}>
                    <h4 style={{ color: headingColor, margin: 0, fontSize: 16, wordBreak: 'break-word' }}>{quiz.title}</h4>
                    <p style={{ fontSize: 12, color: secondaryText, marginTop: 4 }}>
                      Week {quiz.weekNumber} • {quiz.questions.length} questions
                    </p>
                    <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginTop: 6 }}>
                      <span style={{ background: status.color, color: 'white', padding: '2px 10px', borderRadius: 12, fontSize: 11, fontWeight: 'bold' }}>
                        {status.label}
                      </span>
                      {quiz.isPremium && (
                        <span style={{ background: '#ff9800', color: 'white', padding: '2px 10px', borderRadius: 12, fontSize: 11, fontWeight: 'bold' }}>
                          ⭐ Premium
                        </span>
                      )}
                      {quiz.startDate && (
                        <span style={{ background: '#17a2b8', color: 'white', padding: '2px 10px', borderRadius: 12, fontSize: 11 }}>
                  ...
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, justifyContent: 'flex-end' }}>
                    ...
                    <button
                      onClick={() => handleTogglePublish(quiz._id, quiz.isActive)}
                      style={{
                        background: quiz.isActive ? '#dc3545' : '#28a745',
                        color: 'white',
                        padding: '4px 10px',
                        border: 'none',
                        borderRadius: 4,
                    ...
                    <button
                      onClick={() => handleTogglePremium(quiz._id, quiz.isPremium)}
                      style={{
                        background: quiz.isPremium ? '#dc3545' : '#ff9800',
                        color: 'white',
                        padding: '4px 10px',
                        border: 'none',
                        borderRadius: 4,
                    ...
                    >
                      {quiz.isPremium ? 'Remove ⭐' : 'Make ⭐'}
                    </button>
                    <button
                      onClick={() => handleViewResults(quiz._id)}
                      style={{ background: '#17a2b8', color: 'white', padding: '4px 10px', border: 'none', borderRadius: 4, cursor: 'pointer', fontSize: 12, fontWeight: 500 }}
                    >
                      Results
                    </button>
                    <button
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 18, flexWrap: 'wrap', gap: 8 }}>
              ...
              <button
                ...
                style={{ background: '#6c757d', color: 'white', padding: '6px 16px', border: 'none', borderRadius: 6, cursor: 'pointer', fontSize: 13 }}
              >
                Close
              </button>
            </div>
            {selectedQuizResults.length === 0 ? (
              ...
              <div>
                <p style={{ color: secondaryText, marginBottom: 14 }}>
                  Total Attempts: <strong>{selectedQuizResults.length}</strong>
                </p>
                {selectedQuizResults.map((attempt, idx) => (
                  ...
                  >
                    ...
                    <p style={{ margin: 0, fontSize: 13, color: secondaryText }}>
                      {attempt.userId?.email || 'No email'} • Score: <strong>{attempt.score}/{attempt.total}</strong> ({attempt.percentage.toFixed(1)}%) {attempt.passed ? ' ✅' : ' ❌'}
                    </p>
                    <p style={{ margin: 0, fontSize: 11, color: secondaryText }}>
