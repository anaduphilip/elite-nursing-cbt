...
router.get('/current', authenticate, async (req, res) => {
  try {
    ...
    const existingAttempt = await WeeklyQuizAttempt.findOne({ userId: req.user._id, weeklyQuizId: quiz._id });
    ...
    res.json({ success: true, quiz: quizData, alreadyAttempted: !!existingAttempt, isPremium: quiz.isPremium });
  } catch (error) {
...
router.post('/submit', authenticate, async (req, res) => {
  try {
    ...
    const existingAttempt = await WeeklyQuizAttempt.findOne({ userId: req.user._id, weeklyQuizId: quizId });
    if (existingAttempt) return res.status(400).json({ error: 'You have already attempted this weekly quiz.' });
    ...
    if (quiz.startDate && quiz.startDate > now) return res.status(400).json({ error: 'This quiz has not started yet.' });
    if (quiz.endDate && quiz.endDate < now) return res.status(400).json({ error: 'This quiz has already expired.' });
    let score = 0, total = 0;
    quiz.questions.forEach((q, index) => {
      total += q.points || 1;
      if (answers[index] !== undefined && answers[index] === q.correctAnswer) score += q.points || 1;
    });
    ...
    res.json({ success: true, score, total, percentage: percentage.toFixed(1), passed });
  } catch (error) {
...
// Leaderboard for current or specific quiz
